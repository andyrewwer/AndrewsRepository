...
<?php 
	...
	function checkIfUserHasAReservedDirective(){
	    ...
	    while($userRow = $result->fetch_assoc()) {
	    	if ($userRow['Status'] === $_COOKIE["user"]) {
				$query = "UPDATE `d5g9x9d8_testDB`.`Directives` SET  `Status` =  'Available', `StatusName` = 'Available', `DirectiveColour` =  '' WHERE  `Directives`.`DirectiveNumber` ='".$userRow["DirectiveNumber"] ."'";
	...
	function reserveDirective() {
		...
		$query = "UPDATE  `d5g9x9d8_testDB`.`Directives` SET  `Status` =  '". $_COOKIE["user"] ."', `StatusName` = '". $_COOKIE['name'] . "',`DirectiveColour` =  '". $_COOKIE["backroomColour"] ."' WHERE  `Directives`.`DirectiveNumber` =" . $directiveRow["DirectiveNumber"]  . ";";
