<?php
	$con = mysqli_connect("localhost", "d5g9x9d8_user","1qwerty1","d5g9x9d8_testDB") or die(mysql_error());
 	if (isset($_POST['approve'])) {
 		$query = "UPDATE  `d5g9x9d8_testDB`.`Directives` SET  `Status` =  'Available', `StatusName` = 'Available'  WHERE  `Directives`.`DirectiveNumber` = " . $_POST["directiveNumber"] . " ";
 		...
 		$query = "UPDATE  `d5g9x9d8_testDB`.`Directives` SET  `Status` =  'Rejected', `StatusName` = 'Rejected'  WHERE  `Directives`.`DirectiveNumber` = " . $_POST["directiveNumber"] . " ";
 		$result = mysqli_query($con, $query);
