	...
	<?php 
	    ...
	    if ($message === "failure") {
			echo "<script> var alert = new telegramAlert(); alert.render('You did not complete the form!', 'Please finish filling it out!'); </script>";
				...
				echo "<script> var alert = new telegramAlert(); alert.render('Success', 'Keep going!'); </script>";
	    }
...
<?php 
	...
	function checkIfUserHasAReservedDirective(){
	    ...
	    while($userRow = $result->fetch_assoc()) {
	    	if ($userRow['Status'] === $_COOKIE["user"]) {
				$query = "UPDATE `dbilh9sp_UoB`.`Directives` SET  `Status` =  'Available', `StatusName` = 'Available', `DirectiveColour` =  '' WHERE  `Directives`.`DirectiveNumber` ='".$userRow["DirectiveNumber"] ."'";
	...
	function reserveDirective() {
		...
		$query = "UPDATE  `dbilh9sp_UoB`.`Directives` SET  `Status` =  '". $_COOKIE["user"] ."', `StatusName` = '". $_COOKIE['name'] . "',`DirectiveColour` =  '". $_COOKIE["backroomColour"] ."' WHERE  `Directives`.`DirectiveNumber` =" . $directiveRow["DirectiveNumber"]  . ";";
	...
	<table class='directiveTable'> <thead><tr id='head'> <th width=50px>#</th><th width=130px>Character Name</th> <th width=70px>Cabinet</th><th width=110px>Request Type</th> <th  width=540px>Description</th> </tr></thead><tbody class='directiveTable'> <tr> <td width=50px>" . $directiveRow["DirectiveNumber"]. "</td><td width=130px>". $directiveRow["DirectiveSenderName"] ."</td><td width=70px>" . $cabinetArray[$directiveRow["DirectiveCommittee"]] . "</td><td width=110px>" . $directiveRow["DirectiveType"] . "</td><td  width=540px>" . $directiveRow["DirectiveText"] . "</td></tr> </tbody></table>";
