...
//cookie name ="name"
	...
	function submitTelegram() { 
		 		$con = mysqli_connect("localhost", "dbilh9sp_user","1qwerty1","dbilh9sp_NancyMUN") or die(mysql_error());
 ...
 //echo $query. "<br><br>";
	...
	if(isset($_POST['submit'])) { 
		if (!isset($_POST['Sender']) || !isset($_POST['Type']) || !isset($_POST['description'])) {
			...
			while($row = $result->fetch_assoc()) {
				...
				if ($currentTime > $lastDirectiveAndDirectiveTimer) {
					//allow directive
					submitTelegram();
					$queryUpdate = "UPDATE  `dbilh9sp_NancyMUN`.`Users` SET  `LastDirective` =  '". $currentTime ."' WHERE `UserNameID` = '". $_COOKIE['user'] ."'";
	   ...
	   // echo "test";
		...
		VALUES (
		NULL , '$directiveSender', '$directiveSenderName', '$directiveCommittee', '$directiveFrom', '$directiveType', '$directivesDescription', 'Available', 'Available', '', '".$currentDate."')"; 
		$query2 = "UPDATE `dbilh9sp_NancyMUN`.`Responses` SET `ResponseAllowed` = 'f' WHERE `Responses`.`Recipient` = '". $_COOKIE['user']."';";
}
 		// mysqli_query($query);
 // echo $query. "<br><br>";
 		$result = mysqli_query($con, $query);
