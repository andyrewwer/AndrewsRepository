...
//cookie name ="name"
	...
	function submitTelegram() { 
		 		$con = mysqli_connect("localhost", "d5g9x9d8_user","1qwerty1","d5g9x9d8_NancyMUN") or die(mysql_error());
 ...
 //echo $query. "<br><br>";
	...
	if(isset($_POST['submit'])) { 
		if (!isset($_POST['Sender']) || !isset($_POST['Type']) || !isset($_POST['description'])) {
			...
			while($row = $result->fetch_assoc()) {
				...
				if ($currentTime > $lastDirectiveAndDirectiveTimer) {
					//allow directive
					submitTelegram();
					$queryUpdate = "UPDATE  `d5g9x9d8_NancyMUN`.`Users` SET  `LastDirective` =  '". $currentTime ."' WHERE `UserNameID` = '". $_COOKIE['user'] ."'";
	   ...
	   // echo "test";
		...
		VALUES (
		NULL , '$directiveSender', '$directiveSenderName', '$directiveCommittee', '$directiveFrom', '$directiveType', '$directivesDescription', 'Available', 'Available', '', '".$currentDate."')"; 
		$query2 = "UPDATE `d5g9x9d8_NancyMUN`.`Responses` SET `ResponseAllowed` = 'f' WHERE `Responses`.`Recipient` = '". $_COOKIE['user']."';";
}
 		// mysqli_query($query);
 // echo $query. "<br><br>";
 		$result = mysqli_query($con, $query);
