		    ...
		    if ($usernameIsFree === 'true') {
				...
				if (isset($_POST['Chair']) && $_POST['Chair'] === 'Yes') {
					...
					$isChair = 'f';
							...
							$query = "INSERT INTO  `d5g9x9d8_fredmun`.`Users` (
					`UserNameID` ,
					`CharacterName` ,
					`Committee` ,
					`pass` ,
			...
			$result = $con->query("SELECT * FROM Users WHERE `Users`.`id` =".$_POST['delegateID'].";");
		 	...
		 	if ($cabinet === 'backroom') {
				$con = mysqli_connect("localhost", "d5g9x9d8_user","1qwerty1","d5g9x9d8_fredmun") or die(mysql_error());
		 		...
		 		$query = "UPDATE `d5g9x9d8_fredmun`.`Users` SET `isBackroom` = 't', `backroomColour` = '#FCA326' WHERE `Users`.`id` =" .$_POST['delegateID'] ." LIMIT 1";

		 	}else {
				$query =  "UPDATE  `d5g9x9d8_fredmun`.`Users` SET  `Committee` =  '". $cabinet ."',
`isBackroom` =  'f',
`backroomColour` =  '#FCA326', reservedDirective = '' WHERE  `Users`.`id` =".$_POST['delegateID']." LIMIT 1 ;";
				...
				while($row = $changedUserResult->fetch_assoc()) {
					$removeReservedDirective = "UPDATE `d5g9x9d8_fredmun`.`Directives` SET `Status` = 'Available', `StatusName` = 'Available', `DirectiveColour` = '' WHERE `Status` = '".$row['UserNameID']."';";
...
//		 	echo "Cabinet: " . $_POST['cabinet'];
			$name = $_POST['name'];
			if (isset($_POST['Chair']) && $_POST['Chair'] === 'Yes') {
				$query = "UPDATE  `d5g9x9d8_fredmun`.`Users` SET  `CharacterName` =  '". $name."',`Committee` =  '". $_POST['Cabinet'] ."',`isBackroom` = 'f', `BackroomColour` = '#FCA326', reservedDirective = '', `isChair` = 't' WHERE  `Users`.`id` =". $_POST['delegateID'] . " LIMIT 1 ;";			
			}else {
				$query = "UPDATE  `d5g9x9d8_fredmun`.`Users` SET  `CharacterName` =  '". $name."',`Committee` =  '". $_POST['Cabinet'] ."',`isBackroom` = 'f', `BackroomColour` = '#FCA326', reservedDirective = '', `isChair` = 'f' WHERE  `Users`.`id` =". $_POST['delegateID'] . " LIMIT 1 ;";			
			}
		 	$result = mysqli_query($con, $query);
						$location = "Location: createUser.php?message=selectCabinetY"; 
			if ($result) {
...
date_default_timezone_set('Europe/London'); // CDT
			...
			$currentDate = "$date/$month/$year == $hour:$min:$sec";
    					...
    					$query2 = "INSERT INTO  `d5g9x9d8_fredmun`.`Responses` (
						`Recipient` ,
						`RecipientName`,
						`Directive` ,
						`Response` ,
