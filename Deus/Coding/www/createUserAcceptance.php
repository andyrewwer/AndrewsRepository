		    ...
		    if ($usernameIsFree === 'true') {
				...
				if (isset($_POST['Chair']) && $_POST['Chair'] === 'Yes') {
					...
					$isChair = 'f';
							...
							$query = "INSERT INTO  `dbilh9sp_{{CONFERENCE_NAME}}`.`Users` (
					`UserNameID` ,
					`CharacterName` ,
					`Committee` ,
					`pass` ,
			...
			$result = $con->query("SELECT * FROM Users WHERE `Users`.`id` =".$_POST['delegateID'].";");
		 	...
		 	if ($cabinet === 'backroom') {
				$con = mysqli_connect("localhost", "dbilh9sp_user","1qwerty1","dbilh9sp_{{CONFERENCE_NAME}}") or die(mysql_error());
		 		...
		 		$query = "UPDATE `dbilh9sp_{{CONFERENCE_NAME}}`.`Users` SET `isBackroom` = 't', `backroomColour` = '#FCA326' WHERE `Users`.`id` =" .$_POST['delegateID'] ." LIMIT 1";

		 	}else {
				$query =  "UPDATE  `dbilh9sp_{{CONFERENCE_NAME}}`.`Users` SET  `Committee` =  '". $cabinet ."',
`isBackroom` =  'f',
`backroomColour` =  '#FCA326', reservedDirective = '' WHERE  `Users`.`id` =".$_POST['delegateID']." LIMIT 1 ;";
				...
				while($row = $changedUserResult->fetch_assoc()) {
					$removeReservedDirective = "UPDATE `dbilh9sp_{{CONFERENCE_NAME}}`.`Directives` SET `Status` = 'Available', `StatusName` = 'Available', `DirectiveColour` = '' WHERE `Status` = '".$row['UserNameID']."';";
...
//		 	echo "Cabinet: " . $_POST['cabinet'];
			$name = $_POST['name'];
			if (isset($_POST['Chair']) && $_POST['Chair'] === 'Yes') {
				$query = "UPDATE  `dbilh9sp_{{CONFERENCE_NAME}}`.`Users` SET  `CharacterName` =  '". $name."',`Committee` =  '". $_POST['Cabinet'] ."',`isBackroom` = 'f', `BackroomColour` = '#FCA326', reservedDirective = '', `isChair` = 't' WHERE  `Users`.`id` =". $_POST['delegateID'] . " LIMIT 1 ;";			
			}else {
				$query = "UPDATE  `dbilh9sp_{{CONFERENCE_NAME}}`.`Users` SET  `CharacterName` =  '". $name."',`Committee` =  '". $_POST['Cabinet'] ."',`isBackroom` = 'f', `BackroomColour` = '#FCA326', reservedDirective = '', `isChair` = 'f' WHERE  `Users`.`id` =". $_POST['delegateID'] . " LIMIT 1 ;";			
			}
		 	$result = mysqli_query($con, $query);
						$location = "Location: createUser.php?message=selectCabinetY"; 
			if ($result) {
...
date_default_timezone_set('Europe/London'); // CDT
			...
			$currentDate = "$date/$month/$year == $hour:$min:$sec";
    					...
    					$query2 = "INSERT INTO  `dbilh9sp_{{CONFERENCE_NAME}}`.`Responses` (
						`Recipient` ,
						`RecipientName`,
						`Directive` ,
						`Response` ,
