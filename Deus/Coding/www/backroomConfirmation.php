 	...
 	function confirmForm() {
		...
		$responseImage = str_replace("\\", '', $responseImage);
 		...
 		if ($_POST['publicOrPrivate'] === "public") {
 			echo "Public";
	 		$query2 = "INSERT INTO  `d5g9x9d8_fredmun`.`News` (`NewsNumber` ,`NewsTitle` ,`NewsDescription`, `NewsImage` )VALUES (NULL , '" . $responseTitle . "',  '" .  $responseText ."', '". $responseImage . "');";
				...
				foreach($array as $selected){
					if ($first === 0) { 
			 			if ($_POST['responseValue'] === 'on') {
					 		$query2 = "INSERT INTO  `d5g9x9d8_fredmun`.`Responses` (`Recipient`, `RecipientName`,`Directive`,`DirectiveNumber`,`Response` ,`responseDescription` ,`responseID`, `readByDelegate`, `ResponseAllowed`, `MassMessage`, `Timestamp`)VALUES (
	...
	$con = mysqli_connect("localhost", "d5g9x9d8_user","1qwerty1","d5g9x9d8_fredmun") or die(mysql_error());
 	if (isset($_POST['edit'])) {
		...
		// echo "Directive Number: " . $_POST['directiveNumber'] . "<br>";
 		// $query = "UPDATE  `d5g9x9d8_fredmun`.`Directives` SET  `Status` =  'Completed', `StatusName` = 'Completed', `DirectiveColour` =  '#76EE00'  WHERE  `Directives`.`DirectiveNumber` = " . $_POST["directiveNumber"] . " ";
 		$query = "UPDATE  `d5g9x9d8_fredmun`.`Directives` SET  `Status` =  'Completed', `StatusName` = '".$_COOKIE['name']."', `DirectiveColour` =  '#76EE00'  WHERE  `Directives`.`DirectiveNumber` = " . $_POST["directiveNumber"] . " ";
		...
		echo "Directive Number: " . $_POST['directiveNumber'] . "<br>";
 		$query = "UPDATE  `d5g9x9d8_fredmun`.`Directives` SET  `Status` =  'Completed', `StatusName` = 'Failed', `DirectiveColour` =  '#ff0000'  WHERE  `Directives`.`DirectiveNumber` = " . $_POST["directiveNumber"] . " ";
