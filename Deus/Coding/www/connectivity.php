<?php
	...
	if(isset($_POST['submit'])) { 
		...
		$query = "UPDATE  `dbilh9sp_{{CONFERENCE_NAME}}`.`Responses` SET  `readByDelegate` =  't' WHERE  `Responses`.`responseID` =". $_POST['directiveNumber'] . " LIMIT 1 ";
		...
		$con = mysqli_connect("localhost", "dbilh9sp_user","1qwerty1","dbilh9sp_{{CONFERENCE_NAME}}") or die(mysql_error());
		$location = "Location: profile.php?privatemessage=".$_POST['directiveNumber'];
		$query = "UPDATE  `dbilh9sp_{{CONFERENCE_NAME}}`.`Responses` SET  `readByDelegate` =  'f' WHERE  `Responses`.`responseID` =". $_POST['directiveNumber'] . " LIMIT 1 ";
		$result = mysqli_query($con, $query);
		if (!$result) {
				header("Location: profile.php?error=error");
    			echo "ERROR: " . mysqli_error($con);		   		
		...
		if (isset($_POST['title']) && isset($_POST['description']) && isset($_POST['image'])) {
			...
			// header($location);
			$query = "UPDATE  `dbilh9sp_{{CONFERENCE_NAME}}`.`News` SET  `NewsTitle` =  '" .$title ."',
		...
		//THISF SFNSD:KN EF:KN WHAT I"M DOING NOW 
			...
			$query = "DELETE FROM `dbilh9sp_{{CONFERENCE_NAME}}`.`News` WHERE NewsNumber=".$_POST['newsNumber'];
			$result = mysqli_query($con, $query);	
			if (!$result) {
		...
		while ($row = $userResult->fetch_assoc()){
			$query =  "UPDATE `dbilh9sp_{{CONFERENCE_NAME}}`.`GlobalVariables` SET `VariableValue` = '".$_POST['image']. "' WHERE `GlobalVariables`.`VariableName` = 'worldMap';";
			$query2 = " UPDATE `dbilh9sp_{{CONFERENCE_NAME}}`.`GlobalVariables` SET `VariableValue` = '".$row['VariableValue']."' WHERE `GlobalVariables`.`VariableName` = 'worldMapBackup';";
		 	$result = mysqli_query($con, $query);
