<?php
	...
	if(isset($_POST['submit'])) { 
		...
		$query = "UPDATE  `d5g9x9d8_testMUN`.`Responses` SET  `readByDelegate` =  't' WHERE  `Responses`.`responseID` =". $_POST['directiveNumber'] . " LIMIT 1 ";
		...
		$con = mysqli_connect("localhost", "d5g9x9d8_user","1qwerty1","d5g9x9d8_testMUN") or die(mysql_error());
		$location = "Location: profile.php?privatemessage=".$_POST['directiveNumber'];
		$query = "UPDATE  `d5g9x9d8_testMUN`.`Responses` SET  `readByDelegate` =  'f' WHERE  `Responses`.`responseID` =". $_POST['directiveNumber'] . " LIMIT 1 ";
		$result = mysqli_query($con, $query);
		if (!$result) {
				header("Location: profile.php?error=error");
    			echo "ERROR: " . mysqli_error($con);		   		
		...
		if (isset($_POST['title']) && isset($_POST['description']) && isset($_POST['image'])) {
			...
			// header($location);
			$query = "UPDATE  `d5g9x9d8_testMUN`.`News` SET  `NewsTitle` =  '" .$title ."',
		...
		//THISF SFNSD:KN EF:KN WHAT I"M DOING NOW 
			...
			$query = "DELETE FROM `d5g9x9d8_testMUN`.`News` WHERE NewsNumber=".$_POST['newsNumber'];
			$result = mysqli_query($con, $query);	
			if (!$result) {
		...
		while ($row = $userResult->fetch_assoc()){
			$query =  "UPDATE `d5g9x9d8_testMUN`.`GlobalVariables` SET `VariableValue` = '".$_POST['image']. "' WHERE `GlobalVariables`.`VariableName` = 'worldMap';";
			$query2 = " UPDATE `d5g9x9d8_testMUN`.`GlobalVariables` SET `VariableValue` = '".$row['VariableValue']."' WHERE `GlobalVariables`.`VariableName` = 'worldMapBackup';";
		 	$result = mysqli_query($con, $query);
