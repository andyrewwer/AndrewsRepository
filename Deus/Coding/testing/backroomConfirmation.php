				...
				foreach($array as $selected){
					if ($first === 0) { 
			 			if ($_POST['responseValue'] === 'on') {
					 		$query2 = "INSERT INTO  `d5g9x9d8_demo`.`Responses` (`Recipient`, `RecipientName`,`Directive`,`DirectiveNumber`,`Response` ,`responseDescription` ,`responseID`, `readByDelegate`, `ResponseAllowed`, `MassMessage`, `Timestamp`)VALUES (
	...
	$con = mysqli_connect("localhost", "d5g9x9d8_user","1qwerty1","d5g9x9d8_demo") or die(mysql_error());
 	if (isset($_POST['edit'])) {
		...
		// echo "Directive Number: " . $_POST['directiveNumber'] . "<br>";
 		// $query = "UPDATE  `d5g9x9d8_demo`.`Directives` SET  `Status` =  'Completed', `StatusName` = 'Completed', `DirectiveColour` =  '#76EE00'  WHERE  `Directives`.`DirectiveNumber` = " . $_POST["directiveNumber"] . " ";
 		$query = "UPDATE  `d5g9x9d8_demo`.`Directives` SET  `Status` =  'Completed', `StatusName` = '".$_COOKIE['name']."', `DirectiveColour` =  '#76EE00'  WHERE  `Directives`.`DirectiveNumber` = " . $_POST["directiveNumber"] . " ";
		...
		echo "Directive Number: " . $_POST['directiveNumber'] . "<br>";
 		$query = "UPDATE  `d5g9x9d8_demo`.`Directives` SET  `Status` =  'Completed', `StatusName` = 'Failed', `DirectiveColour` =  '#ff0000'  WHERE  `Directives`.`DirectiveNumber` = " . $_POST["directiveNumber"] . " ";
