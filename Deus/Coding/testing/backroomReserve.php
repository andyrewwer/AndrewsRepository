...
<?php 
	...
	function checkIfUserHasAReservedDirective(){
	    ...
	    while($userRow = $result->fetch_assoc()) {
	    	if ($userRow['Status'] === $_COOKIE["user"]) {
				$query = "UPDATE `dbilh9sp_demo`.`Directives` SET  `Status` =  'Available', `StatusName` = 'Available', `DirectiveColour` =  '' WHERE  `Directives`.`DirectiveNumber` ='".$userRow["DirectiveNumber"] ."'";
	...
	function reserveDirective() {
		...
		$query = "UPDATE  `dbilh9sp_demo`.`Directives` SET  `Status` =  '". $_COOKIE["user"] ."', `StatusName` = '". $_COOKIE['name'] . "',`DirectiveColour` =  '". $_COOKIE["backroomColour"] ."' WHERE  `Directives`.`DirectiveNumber` =" . $directiveRow["DirectiveNumber"]  . ";";
	...
	<table id='directives' class='reserveTable'> <thead><tr> <th width='5%'>#</th> <th width='13%'>Character Name</th><th width='7%'>Directive Type</th> <th width='61%'>Cabinet</th></tr></thead><tbody class='reserveTable'> <tr> <td width=5%>" . $directiveRow["DirectiveNumber"]. "</td><td width=13%>". $directiveRow["DirectiveSenderName"] ."</td><td width=11%>" . $directiveRow["DirectiveType"] . "</td><td  width=61%>" . $directiveRow["DirectiveText"] . "</td></tr> </tbody></table>";		

	// <table class='reserveTable'> <thead><tr id='head'> <th width=5%>#</th><th width=13%>Character Name</th> <th width=7%>Cabinet</th><th width=11%>Request Type</th> <th  width=54%>Description</th> </tr></thead><tbody> <tr> <td width=5%>" . $directiveRow["DirectiveNumber"]. "</td><td width=13%>". $directiveRow["DirectiveSenderName"] ."</td><td width=7%>" . $cabinetArray[$directiveRow["DirectiveCommittee"]] . "</td><td width=11%>" . $directiveRow["DirectiveType"] . "</td><td  width=54%>" . $directiveRow["DirectiveText"] . "</td></tr> </tbody></table>";
