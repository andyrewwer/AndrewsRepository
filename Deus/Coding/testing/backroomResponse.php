...
// /// HERE WE ARE MAKING THE BUTTONS FOR SHOW ALL OR ONLY SHOW COMPLETED 
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				$element = $element . "> <td width='8%'>" . $row["DirectiveNumber"]. "</td><td width='10%'>" . $row["Timestamp"]. "</td><td width='10%'>". $row["DirectiveSenderName"] ."</td><td width='10%'>" . $cabinetArray[$row["DirectiveCommittee"]] . "</td><td width='8%'>" . $row["DirectiveType"] . "</td><td width =10%>" . $row["DirectiveFrom"] . "</td><td width='35%'>" . $directiveText . "</td><td width='12%'>". $row['StatusName'] ."</td></tr>"; // Add directiveFROM
				$myArray[] = $element;
			}
			...
			echo "<table id='directives' class='responseTable'> <thead><tr> <th width='8%'>#</th> <th width='10%'>Timestamp</th><th width='10%'>Character Name</th> <th width='10%'>Cabinet</th><th width='8%'>Request Type</th><th width='10%'>From </th><th width='35%'>Description</th> <th width='12%'>Status</th></tr></thead><tbody id='directives' class='responseTable'>";		
